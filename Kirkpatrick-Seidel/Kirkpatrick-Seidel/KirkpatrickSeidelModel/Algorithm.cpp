#ifndef KirkpatrickSeidel
#define KirkpatrickSeidel
...
struct KirkpatrickSeidelAlgorithm {
	...
	vector<Point> get_T(Point firstPoint,Point secondPoint,
						const vector<Point> &points,
						bool flag) {
		vector<Point> upper_T;
		double slope = (firstPoint.y - secondPoint.y) / (firstPoint.x - secondPoint.x);
		for(int i = 0; i < points.size(); i++) {
			Point currentPoint = points.at(i);
			if(currentPoint.x > firstPoint.x && currentPoint.x < secondPoint.x) {
			  double currentSlope = (firstPoint.y - currentPoint.y) / (firstPoint.x - currentPoint.x);
			  if(!flag) {
				  if(currentSlope > slope) {
					 upper_T.push_back(currentPoint);
				  }
			  }
			  else {
				if(currentSlope < slope) {
		  ...
		  upper_T.push_back(firstPoint);
		  upper_T.push_back(secondPoint);

		return upper_T;
	}

//	PointPair getUpperBridge(const vector<Point> &points,
//							 double median) {
//
...
//		vector<Point> candidates(numberOfPoints);
//		vector<PointPair> pairs(numberOfPoints / 2 + 1);
//		const size_t numberOfPairs = pairs.size();
//		if(points.size() % 2 == 0 ) {
//			for(int i=0; i < numberOfPoints; i += 2) {
//				const Point firstPoint = points.at(i);
//				const Point secondPoint = points.at(i + 1);
//				pairs.push_back(
//					PointPair(firstPoint, secondPoint)
//				);
//			}
//		}
//		else {
//			candidates.push_back(points.at(0));
...
//		}
//		
//		int numberOfSlopes = numberOfPairs;
//		double slopes[numberOfSlopes];
//		for(int i = 0; i < numberOfPairs; i++) {
//
//			const Point firstPoint = pairs.at(i).first;
//			const Point secondPoint = pairs.at(i).second;
//			const double x1 = firstPoint.x;
//			const double x2 = secondPoint.x;
//			const double y1 = firstPoint.x;
//			const double y2 = secondPoint.y;
//
//			if(x1 == x2){
//				candidates.push_back( y1 > y2 ? firstPoint : secondPoint);
//				slopes[i] = INT_MAX;
//			}
//			else {
...
public:
	KirkpatrickSeidelAlgorithm(const vector<Point> inputVector) {
		points = inputVector;
	}
	
	vector<Point> giveConvexHullOf(const vector<Point> &points) {
		...
		Point minUpperHullPoint, maxUpperHullPoint, minLowerHullPoint, maxLowerHullPoint;
		minUpperHullPoint = maxUpperHullPoint = minLowerHullPoint = maxLowerHullPoint = points.at(0);
		
	  for(int i = 1; i < points.size(); i++) {
		  Point currentPoint = points.at(i);
		  
		  if(currentPoint.x < minLowerHullPoint.x) {
			  minLowerHullPoint = currentPoint;
			  minUpperHullPoint = currentPoint;
		  }
		  else if(currentPoint.x > maxLowerHullPoint.x){
			  maxLowerHullPoint = currentPoint;
			  maxUpperHullPoint = currentPoint;
		  }
		  else if(currentPoint.x == minLowerHullPoint.x) {
			if(currentPoint.y > minUpperHullPoint.y) {
				minUpperHullPoint = currentPoint;
			}
			else if(currentPoint.y < minLowerHullPoint.y){
				minLowerHullPoint = currentPoint;
			}
		  }
		  else if(currentPoint.x == maxLowerHullPoint.x) {
			if(currentPoint.y > maxUpperHullPoint.y) {
				maxUpperHullPoint = currentPoint;
			}
			else if(currentPoint.y < maxLowerHullPoint.y){
				maxLowerHullPoint = currentPoint;
			}
		  }
	  }
